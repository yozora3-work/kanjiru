...
function queryFormer(options, cardType, property) {
  ...
  if (property === "kanji") {
    return {
      query: `SELECT * FROM ${cardType} WHERE kanji = ? OR kanji_translation LIKE ? OR kanji_readingOn LIKE ? OR kanji_readingKun LIKE ? `,
      params: [
        options.kanji,
        `%${options.kanji}%`,
        `%${options.kanji}%`,
...
export async function getData(db, options) {
  ...
  //handle odd query
  if (options.kanji) {
    const test = queryFormer(options, "kanjiData", "kanji");
    const tt = await fetchAll(db, test.query, test.params);
    return tt;
  }
  // Handle ID query
  if (options.id) {
